  ...
  try {
    if (activeTab === 'notes') {
      ...
      if (statsResponse.success) {
        setTaskStats(
          typeof statsResponse.data === 'object' && statsResponse.data
            ? statsResponse.data
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        {isEditing ? (
          <input
            ...
            onBlur={handleSave}
            className="flex-1 text-lg font-semibold bg-transparent border-b-2 border-white/30 focus:border-indigo-400 outline-none text-gray-900 placeholder-gray-600"
            autoFocus
      ...
      {isEditing ? (
        <textarea
          ...
          onBlur={handleSave}
          className="w-full min-h-[100px] bg-transparent text-gray-800 resize-none outline-none placeholder-gray-600"
...
function TaskCard({
  ...
  const handleSave = () => {
    if (title.trim() !== task.title || description.trim() !== task.description) {
      onUpdate(task.id, { title: title.trim() || 'Untitled', description: description.trim() });
    }
    setIsEditing(false);
  };
  ...
  const statusColors = {
    ...
    in_progress: 'bg-blue-500/30 text-blue-200 border border-blue-400/30',
    completed: 'bg-green-500/30 text-green-200 border border-green-400/30',
  };

  const priorityColors = {
    low: 'bg-gray-500/30 text-gray-200 border border-gray-400/30',
    medium: 'bg-orange-500/30 text-orange-200 border border-orange-400/30',
    high: 'bg-red-500/30 text-red-200 border border-red-400/30',
  };
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      exit={{ opacity: 0, y: -20 }}
      className={`task-card rounded-2xl p-6 backdrop-blur-xl bg-white/10 border border-white/20 shadow-lg hover:shadow-[0_8px_32px_rgba(0,0,0,0.3)] transition-all duration-300 ${
        ...
        <div className="flex items-start space-x-3 flex-1">
          ...
          <div className="flex-1 min-w-0">
            {isEditing ? (
              ...
              <h3 className={`text-lg font-semibold text-white mb-1 ${task.status === 'completed' ? 'line-through opacity-60' : ''}`}>
                {task.title}
              </h3>
            )}
            ...
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              <span className={`px-3 py-1 rounded-full text-xs font-medium backdrop-blur-md ${statusColors[task.status]}`}>
                {task.status.replace('_', ' ')}
              </span>
              <span className={`px-3 py-1 rounded-full text-xs font-medium backdrop-blur-md ${priorityColors[task.priority]}`}>
                {task.priority}
              </span>
              {task.due_date && (
